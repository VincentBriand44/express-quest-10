...
usersRouter.post('/', async (req, res) => {
  ...
  const hashPass = await User.hashPassword(password).then(hashedPassword => {
    delete reqBody['password']
    reqBody.hashedPassword = hashedPassword
    return reqBody
  })
  User.findByEmail(email)
    .then(existingUserWithEmail => {
      if (existingUserWithEmail) return Promise.reject('DUPLICATE_EMAIL')
      validationErrors = User.validate(hashPass)
      if (validationErrors) return Promise.reject('INVALID_DATA')
      return User.create(hashPass)
    })
    .then(user => {
